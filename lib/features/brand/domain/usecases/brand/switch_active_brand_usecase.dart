...
class SwitchActiveBrandUseCase implements UseCase<Brand, SwitchActiveBrandParams> {
  ...
  Future<Either<Failure, Brand>> call(SwitchActiveBrandParams params) async {
    try {
      // Log execution
      print('SwitchActiveBrandUseCase: Switching to brand ${params.brandId} for user $_currentUserId');
      ...
      return result.fold(
        (failure) {
          print('SwitchActiveBrandUseCase: Failed to switch brand - ${failure.message}');
          return Left(failure);
        },
        (brand) async {
          // Log success
          print('SwitchActiveBrandUseCase: Successfully switched to brand "${brand.name}" (${brand.id})');

          // Handle local storage result
          saveResult.fold(
            (storageFailure) {
              print('SwitchActiveBrandUseCase: Warning - Failed to save active brand ID locally: ${storageFailure.message}');
      ...
      return Left(ServerFailure(message: 'Gagal switch brand: ${e.toString()}'));
    }
  }
    ...
    try {
      ...
      return brandResult.fold(
        (failure) {
          ...
          return ServerFailure(message: 'Gagal memeriksa akses brand: ${failure.message}');
        },
        (brand) {
          ...
          if (brand.ownerId != _currentUserId) {
            ...
            print('SwitchActiveBrandUseCase: User is not owner, checking accessible brands...');

            // Cek apakah brand ada di accessible brands
            _repository.getAccessibleBrands().then(
              (accessibleResult) => accessibleResult.fold(
                (failure) => print('SwitchActiveBrandUseCase: Could not verify access: ${failure.message}'),
                (brands) {
                  final hasAccess = brands.any((b) => b.id == brandId);
                  if (!hasAccess) {
                    print('SwitchActiveBrandUseCase: User does not have access to brand $brandId');
                  } else {
                    print('SwitchActiveBrandUseCase: User has access to brand $brandId');
                  }
                },
              ),
            );
      ...
      return ServerFailure(message: 'Gagal memeriksa akses brand: ${e.toString()}');
    }
  }
  ...
  Future<Either<Failure, void>> _saveActiveBrandId(String brandId) async {
    try {
      ...
      return Left(CacheFailure(message: 'Gagal menyimpan active brand ID: ${e.toString()}'));
    }
  }
}