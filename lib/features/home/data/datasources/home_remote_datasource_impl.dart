...
class HomeRemoteDataSourceImpl implements HomeRemoteDataSource {
  ...
  Future<Either<Failure, List<MenuItemModel>>> getMenuItems() async {
    try {
      ...
      return Left(NetworkFailure(
          message: 'Failed to load menu items: ${e.toString()}'));
    }
  }

  @override
  Future<Either<Failure, UserDashboardModel>> getUserDashboard(
      String userId) async {
    try {
      ...
      return Left(NetworkFailure(
          message: 'Failed to load user dashboard: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, List<MenuItemModel>>> getFeaturedMenuItems() async {
    try {
      ...
      _logger.info(
          'Successfully loaded ${featuredItems.length} featured menu items');

      return Right(featuredItems);
    } catch (e) {
      _logger.error('Error getting featured menu items: $e');
      return Left(NetworkFailure(
          message: 'Failed to load featured menu items: ${e.toString()}'));
    }
  }
  ...
  Future<Either<Failure, List<MenuItemModel>>> getMenuItemsByCategory(
    ...
    try {
      ...
      _logger.info(
          'Successfully loaded ${filteredItems.length} menu items for category: $category');

      return Right(filteredItems);
    } catch (e) {
      _logger.error('Error getting menu items by category: $e');
      return Left(NetworkFailure(
          message: 'Failed to load menu items for category: ${e.toString()}'));
    }
  }
